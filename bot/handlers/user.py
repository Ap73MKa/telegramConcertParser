...
from bot.database import create_user_city, get_user_by_id_or_none, delete_outdated_concerts, \
...
async def __handle_response_main(msg: Message, state: FSMContext) -> None:
    ...
    if 'предыдущие запросы' in msg.text.lower():
        cities = [city.city.abb for city in get_all_city_of_user_or_none(get_user_by_id_or_none(msg.from_user.id))]
        ...
        await msg.bot.send_message(msg.from_user.id, text=Messages.get_before_list(),
...
async def __handle_response_city(msg: Message, state: FSMContext) -> None:
    ...
    if await _handle_home_request(msg, state) or await _handle_city_check_request(msg):
        return
    await state.set_state(_MenuStates.MAIN)
    await msg.bot.send_message(user_id, text=Messages.get_error_city(), reply_markup=MarkupKb.get_main(user))
