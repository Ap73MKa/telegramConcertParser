...
async def handle_response_main(msg: Message, state: FSMContext) -> None:
    ...
    if "предыдущие запросы" in msg.text.lower():
        ...
        await msg.bot.send_message(
            msg.from_user.id,
            text=Messages.get_before_list_msg(),
    ...
    if msg.text in ("⬅️", "➡️"):
        ...
        await msg.bot.send_message(
            ...
            reply_markup=get_city_list_keyboard(
                get_user_by_id(msg.from_user.id), direction
            ),
        )
        return
...
async def handle_city_check_request(msg: Message) -> bool:
    ...
    if close[1] >= 80 and len(msg.text) >= 3:
        ...
        await msg.bot.send_message(
            msg.from_user.id, Messages.get_concert_list(city.abb)
        )
        return True
    return False
...
async def update(msg: Message) -> None:
    ...
    await msg.bot.send_message(
        msg.from_user.id, Messages.get_update_time(perf_counter() - timer)
    )
...
async def start(msg: Message, state: FSMContext) -> None:
    ...
    await msg.bot.send_message(
        user_id,
        text=Messages.get_welcome_msg(msg.from_user.full_name),
        reply_markup=get_main_keyboard(get_user_by_id(user_id)),
    )


async def welcome(msg: Message, state: FSMContext) -> None:
    with open(PathManager.get("assets/banner.jpg"), "rb") as photo:
        await state.set_state(MenuStates.main_menu)
